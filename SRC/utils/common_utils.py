...
def sep_column_dtypes(df):
    ...
    lg.debug(f"Boolean Columns: {bool_columns}")
    lg.debug(f"Numerical Columns: {numeric_columns}")
    if len(categories_columns) + len(bool_columns) + len(numeric_columns) == df.shape[1]:
        lg.info("All columns are categorical, boolean or numerical")
    else :
        lg.info("Some columns are not categorical, boolean or numerical")
        raise Exception("Some columns are not categorical, boolean or numerical")

    return categories_columns, bool_columns, numeric_columns 
